...
from .config import OUTPUT_DIR, LOGS_DIR, DEFAULT_MAX_RETRIES, DEFAULT_INITIAL_DELAY
...
def download_youtube_audio_with_metadata(
    url: str,
    ...
    while retries <= max_retries:
        try:
            ...
            if retries <= max_retries:
                ...
                log_and_print(
                    f"Retrying download for {url} in {delay} seconds (Attempt {retries}/{max_retries})...",
                    "warning",
                )
                time.sleep(delay)
            else:
                log_and_print(
                    f"🛑 Max retries ({max_retries}) exceeded for {url}. Giving up.",
                    "error",
                )
                failed += 1
...
def download_serial(urls: list) -> None:
    ...
    end_time = time.time()
    log_and_print(
        f"\n🎯 Download Summary:\n   ✅ Successful: {total_successful}\n   ❌ Failed: {total_failed}\n   ⌛ Time taken to download {len(urls)}: {end_time - start_time:.2f} seconds",
        "info",
    )
